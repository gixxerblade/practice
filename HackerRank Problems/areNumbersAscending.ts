const areNumbersAscending = (s: string): boolean => {
  let count = 1
  const arr = s.split(' ').filter(Number).map(Number)
  arr.forEach((num, i, ar) => {
    if (num < ar[i + 1]) {
      count++
    }
  })
  return count === arr.length
}

const s = "1 box has 3 blue 4 red 6 green and 12 yellow marbles"
const s1 = "sunset is at 7 51 pm overnight lows will be in the low 50 and 60 s"
console.log(areNumbersAscending(s));
console.log(areNumbersAscending(s1));
